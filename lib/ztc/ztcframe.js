 ...
 * @protected
 */
function ZtcFrame(opCodeGroup, opCode, buffer) {
 ...
 * @param {!ZtcParser} buffer - the buffer to create the frame for
 * @returns {ZtcFrame} a ZtcFrame (or subclass of it)
 */
ZtcFrame.create = function (opCodeGroup, opCode, parser) {
    try {
        var Ctor = ZtcFrame._registry[opCodeGroup][opCode];
        return new Ctor(parser);
    } catch (ex) {
        // No specific ZtcFrame is registered.
        return new ZtcFrame(opCodeGroup, opCode, buffer);
    }
};
